...
const ShortenBox = ({ getData, input, handleInput, errorMessage, loading }) => {
  return (
    <>
      <div className="shorten-box">
        ...
        <button className="shorten-box__btn" onClick={getData} disabled={loading}>
          {loading ? " Working on it" : "Shorten It!"}
        </button>
      </div>
    </>
  );
